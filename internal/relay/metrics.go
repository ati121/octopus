...
// RelayMetrics 负责最终的统计收集与进程内日志更新。
type RelayMetrics struct {
	...
	// logSaved 标记本次请求是否已经写入终态。StartLog 会把记录挂进 op 的在途表，
...
// FinalizeIfUnsaved 兜底写入一条终态日志，用 defer 挂在 StartLog 之后。
//
// StartLog 把记录挂进 op 的在途表后，只有终态更新会把它摘掉；任何没走到 Save 的
// 提前返回，或被 gin.CustomRecovery 吞掉的 panic，都会让这条记录永久滞留——既是
// 内存泄漏，也会让日志页一直挂着一条「处理中」的幽灵记录。已完成时是空操作。
