...
func TestRelayLogEnsureIndexesNoopsWithoutLegacyTable(t *testing.T) {
	ctx := setupSiteOpTestDB(t)
	if err := RelayLogEnsureIndexesSync(ctx); err != nil {
		t.Fatalf("RelayLogEnsureIndexesSync failed: %v", err)
	}
	if dbpkg.GetDB().Migrator().HasTable("relay_logs") {
		t.Fatal("index helper must not recreate legacy relay_logs table")
	}
}
