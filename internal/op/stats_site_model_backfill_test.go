...
// TestStatsSiteModelBackfillIgnoresHeavyContent 通过测试专用的旧版表插入一批
// 携带巨型 request_content / response_content 的 relay_logs，验证回填只读取
// 聚合所需列，且历史聚合语义保持不变。生产启动迁移会在此任务前删除该表；
// 这里显式创建它，只覆盖仍需兼容的旧数据库路径。
func TestStatsSiteModelBackfillIgnoresHeavyContent(t *testing.T) {
	...
	if err := db.AutoMigrate(&model.RelayLog{}); err != nil {
		t.Fatalf("create legacy relay_logs table failed: %v", err)
	}
	t.Cleanup(func() {
		_ = db.Migrator().DropTable("relay_logs")
	...
	sites := []model.Site{{ID: 1, Name: "site-1", Platform: model.SitePlatformNewAPI, BaseURL: "https://a.example", Enabled: true}}
	...
	hour := int(now / 3600)
	logs := []model.RelayLog{
		{ID: 1, Time: now, ChannelId: 101, ActualModelName: "gpt-4o", RequestModelName: "gpt-4o", Error: "", RequestContent: heavy, ResponseContent: heavy,
			Attempts: []model.ChannelAttempt{{ChannelID: 101, ModelName: "gpt-4o", Status: model.AttemptSuccess}, {ChannelID: 202, ModelName: "claude", Status: model.AttemptFailed}}},
		{ID: 2, Time: now, ChannelId: 202, ActualModelName: "claude", RequestModelName: "claude", Error: "timeout", RequestContent: heavy, ResponseContent: heavy},
		{ID: 3, Time: now, ChannelId: 999, ActualModelName: "ghost", RequestModelName: "ghost", RequestContent: heavy},
	}
	...
	if !done {
		t.Fatalf("expected legacy backfill flag to be marked complete")
	}
	...
	for k := range got {
		if k.model == "ghost" {
			t.Fatalf("unbound channel was incorrectly aggregated: %+v", k)
		}
	}
}
