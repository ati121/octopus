...
- 设置页新增「上游请求」卡片，为发往上游的请求补齐全局与按模型两层配置，点「配置」进弹窗编辑四个区块：**全局请求头**（对所有上游请求追加）、**模型请求头规则**、**全局参数覆盖**（JSON 对象，深合并进请求体）、**模型参数覆盖规则**。模型规则用逗号分隔的通配符匹配（如 `gpt-4*,o3-*`），`*` 可跨 `/`（`openai/*` 能命中 `openai/gpt-4o`），大小写不敏感，按**解析后的上游模型名**匹配（即分组模型映射之后的名字）。四层合并优先级为：请求体 < 全局 < 模型规则 < 渠道覆盖，渠道级的自定义请求头与参数覆盖仍然最高。
- 全局层与模型规则层的参数覆盖禁止包含 `model` 字段，后端直接以 400 拒绝：覆盖作用在最终请求体字节上，若能改写 `model` 会静默击穿分组的模型映射结果，并让熔断与负载均衡按模型统计的口径错位。渠道级参数覆盖为兼容既有配置不加此限制。
- 生效范围为主链路 HTTP：标准协议转换与 Anthropic / Responses 同协议透传，覆盖 chat / responses / anthropic / gemini / embeddings 等常规流量。图片接口（`/v1/images/*`）、`/responses/compact`、WebSocket 与分组健康探活本次不生效，弹窗内有对应说明。未配置时完全不读取、不改写请求体，透传路径的字节保真与 Anthropic prompt-cache 命中不受影响。
- **行为变更**：渠道级参数覆盖（渠道「高级设置」里的「参数覆盖」）由顶层浅合并改为深合并，四层语义统一。嵌套对象此前整体替换、现在逐层合并——例如请求体为 `{"thinking":{"type":"enabled","budget_tokens":1024}}`、覆盖为 `{"thinking":{"budget_tokens":4096}}` 时，新行为得到 `{"thinking":{"type":"enabled","budget_tokens":4096}}`（保留 `type`），旧行为会把 `type` 一并丢掉。若原先正是靠「整体替换嵌套对象」来清掉子键，需要改写该渠道的覆盖内容。数组仍为整体替换（`tools` / `messages` 语义上只能如此）。
